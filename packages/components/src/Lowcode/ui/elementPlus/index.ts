...
import { defaultProps } from "../../props/utils"
type MapRaw = { raw: RawComponent; node: VNode | Component }

const modules = import.meta.glob("./config/el-*.ts", { eager: true })
...
const transformModules = (modules: Record<string, any>, map: Map<string, MapRaw>) => {
  return Object.entries(modules)
    ...
    .forEach(([path, module]) => {
      ...
      const raw = module.default as RawComponent
      Object.assign(raw.props, defaultProps())
      map.set(label, {
        raw,
        node,
      })
